...
- [ ] Использовать модельный класс Store для создание модели всего приложения
- [ ] Представлениям (view) корзинки и списку в корзине добавить атрибут data-action с действием по клику
- [ ] Переработка CartController по аналогии с MenuController
  - [ ] Контроллер больше не занимается render, метод переименован в init
  - [ ] Изначальный render происходит в инициализации всей программы
  - [ ] Все клики обрабатываются одним handleClick, внутри выбирается действие по data-action
  - [ ] Реализовать действия для CartController:
    - [ ] удалять элементы из модели корзины и бросать событие updated.cart
